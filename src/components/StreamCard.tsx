...
function StreamCard({ url, style }: any) {
  const typedTextRef = useRef(null);
  const startTimeRef = useRef(0);

  useEffect(() => {
    const options = {
      strings: ["Imagine a black ceramic bowl set on a rustic wooden table, showcasing the enticing colors and textures. Tender, succulent pieces of boneless chicken are nestled the aroma of aromatic spices wafts through the air."],
      typeSpeed: 10,
      backSpeed: 20,
      showCursor: false,
      onStart: () => {
